...
void BH1750_Start()
{
	...
	IIC_SDA_OUT_1 = 1;       //拉高数据线
	IIC_SCL_1 = 1;           //拉高时钟线
	delay_us(5);           //延时
	IIC_SDA_OUT_1 = 0;       //产生下降沿
	delay_us(5);           //延时
	IIC_SCL_1 = 0;           //拉低时钟线
}
...
{
	IIC_OUTPUT_MODE_SET(); 	//配置输出模式
	IIC_SDA_OUT_1 = 0;        //拉低数据线
	IIC_SCL_1 = 1;            //拉高时钟线
	delay_us(5);            //延时
	IIC_SDA_OUT_1 = 1;        //产生上升沿
	delay_us(5);            //延时
}
...
void BH1750_SendACK(char ack)
{
	IIC_OUTPUT_MODE_SET(); 	  //配置输出模式
	IIC_SCL_1 = 0;              //拉低时钟线
	if(ack) IIC_SDA_OUT_1 = 1; 
	else IIC_SDA_OUT_1 = 0;     //写应答信号
	delay_us(2);              //延时
	IIC_SCL_1 = 1;              //拉高时钟线
	delay_us(5);              //延时
	IIC_SCL_1 = 0;              //拉低时钟线
...
void BH1750_SendByte(u8 data)
{
	u8 i;
	IIC_OUTPUT_MODE_SET();
	IIC_SCL_1 = 0;				      //拉低时钟线, 数据准备发送
	delay_us(2);					  //延时
	for (i = 0;i < 8; i++)			  //从高位开始一位一位地传送
	{
		if(data&0x80) IIC_SDA_OUT_1 = 1;//送数据口
		else IIC_SDA_OUT_1 = 0;
		data <<= 1;				      //移出数据的最高位
		IIC_SCL_1 = 1;				  //拉高时钟线,发送数据
		delay_us(2);				  //延时
		IIC_SCL_1 = 0;			      //拉低时钟线，数据发送完毕
		delay_us(2);				  //延时
	}
}
...
u8 BH1750_RecvByte()
{
	...
	for (i = 0; i < 8; i++)   		//8位计数器
	{
		IIC_SCL_1 = 0;				//置时钟线为低，准备接收数据位
		delay_us(2);			    //时钟低电平周期大于4.7μs
		IIC_SCL_1 = 1;			   	//置时钟线为高使数据线上数据有效，主机开始读数据，从机不能再改变数据了，即改变SDA的电平
		data <<= 1; 
		IIC_SCL_1 = 1;			 	//拉高时钟线
		if(IIC_SDA_IN_1) data |= 0x01;//读数据               
		delay_us(1);				 				
	}
	IIC_SCL_1 = 0;
	return data;
